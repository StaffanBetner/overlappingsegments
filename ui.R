...
shinyUI(fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      fileInput('file', 'Upload one or more CSV File(s) with Chromosome Browser Results (Max. Size 30 MB)',
                accept=c('text/csv', 'text/comma-separated-values,text/plain', '.csv'), multiple = T),
      fileInput('file2', '(Optional) Upload one or more CSV File(s) with Matches List(s) (Max. Size 30 MB)',
                accept=c('text/csv', 'text/comma-separated-values,text/plain', '.csv'), multiple = T),
